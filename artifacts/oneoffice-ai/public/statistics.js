...
const firebaseConfig = {
  apiKey: "AIzaSyBnESMGkgj17bQNVXOD3fW7w5noxNXipY",
...
// Keep the same public Firebase web configuration as the main app. It is
// intentionally not a secret; authentication is still enforced by the API.
...
function lineChart(rows) {
  if (!rows?.length) return '<div class="empty">30 kunlik tarixiy maʼlumot hali mavjud emas.</div>';
...
function youtubeView(data) {
  ...
  app.innerHTML = `
    <section class="hero"><div class="heroRow"><div class="channel">${first.thumbnailUrl ? `<img class="avatar" src="${esc(first.thumbnailUrl)}" alt=""/>` : `<div class="avatar"></div>`}<div><h2>${esc(first.title || "YouTube")}</h2><p>${esc(first.customUrl || first.channelId || "")} · ${data.rangeDays || 30} kunlik analytics</p></div></div><div class="updated">Yangilandi: ${new Date(data.updatedAt).toLocaleString("uz-UZ")}</div></div></section>
    <div class="grid">
      ${card("Jami ko‘rishlar", n(t.viewCount))}
      ${card("Obunachilar", n(t.subscriberCount), `${netSubs >= 0 ? "+" : ""}${n(netSubs)} net · 30 kun`)}
      ${card("Videolar", n(t.videoCount))}
      ${card("30 kunlik views", n(t.views30d))}
      ${card("Watch time", minutes(t.watchMinutes30d), "30 kun")}
      ${card("Likes", n(t.likes30d), "30 kun")}
      ${card("Comments", n(t.comments30d), "30 kun")}
      ${card("Shares", n(t.shares30d), "30 kun")}
    </div>
    <div class="two"><section class="panel"><div class="sectionTitle"><h3>Views · kunlik</h3><span class="muted">${esc(first.analytics?.startDate || "")} — ${esc(first.analytics?.endDate || "")}</span></div>${lineChart(daily)}</section><section class="panel"><h3>Audience & engagement</h3><table class="table"><tbody><tr><th>Average view duration</th><td>${duration(first.analytics?.totals?.averageViewDuration)}</td></tr><tr><th>Engaged views</th><td>${n(first.analytics?.totals?.engagedViews)}</td></tr><tr><th>Subscribers gained</th><td>${n(first.analytics?.totals?.subscribersGained)}</td></tr><tr><th>Subscribers lost</th><td>${n(first.analytics?.totals?.subscribersLost)}</td></tr><tr><th>Channel views</th><td>${n(first.viewCount)}</td></tr><tr><th>Public videos</th><td>${n(first.videoCount)}</td></tr></tbody></table></section></div>
    <section class="panel"><div class="sectionTitle"><h3>Connected YouTube channels</h3><span class="muted">API orqali real maʼlumot</span></div><table class="table"><thead><tr><th>Channel</th><th>Views</th><th>Subscribers</th><th>Videos</th><th>Status</th></tr></thead><tbody>${top.map(c => `<tr><td>${esc(c.title || c.channelId)}</td><td>${n(c.viewCount)}</td><td>${n(c.subscriberCount)}</td><td>${n(c.videoCount)}</td><td>${c.error ? `<span style="color:#fda4af">${esc(c.error)}</span>` : '<span class="pill">Live API</span>'}</td></tr>`).join("")}</tbody></table></section>
    ${first.analytics?.error ? `<div class="notice" style="margin-top:14px">YouTube Data API channel statistics ishladi, lekin Analytics API ayrim hisobotlari qaytmadi: ${esc(first.analytics.error)}</div>` : ""}
  `;
}

async function telegramView() {
  const [live, combined] = await Promise.all([
    api("/api/telegram-mtproto/stats/live"),
    api("/api/stats/dashboard/combined?granularity=day"),
  ]);
...
async function simpleConnectorView(kind) {
  const [stats, accounts] = await Promise.all([
    api(`/api/connectors/statistics/${kind}`),
    api(`/api/connectors/${kind}`),
  ]);
