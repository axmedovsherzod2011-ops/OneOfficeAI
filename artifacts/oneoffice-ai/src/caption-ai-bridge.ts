...
function fieldByPlaceholder(selector: string, placeholder: string): HTMLInputElement | HTMLTextAreaElement | null {
  return Array.from(document.querySelectorAll<HTMLElement>(selector)).find((el) => el.getAttribute("placeholder") === placeholder) as HTMLInputElement | HTMLTextAreaElement | null;
}
...
function injectStyles() {
  ...
  style.id = STYLE_ID;
  style.textContent = `#${BUTTON_ID}{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;margin-top:8px;padding:9px 12px;border-radius:10px;border:1px solid rgba(139,92,246,.35);background:linear-gradient(90deg,rgba(139,92,246,.16),rgba(59,130,246,.16));color:#c4b5fd;font-size:12px;font-weight:600;cursor:pointer;transition:.2s}#${BUTTON_ID}:hover{background:linear-gradient(90deg,rgba(139,92,246,.25),rgba(59,130,246,.25));color:white;border-color:rgba(139,92,246,.6)}#${BUTTON_ID}:disabled{opacity:.55;cursor:not-allowed}`;
  document.head.appendChild(style);
}

async function generateCaption(button: HTMLButtonElement, textarea: HTMLTextAreaElement) {
  const user = auth.currentUser;
  if (!user) return textarea.focus();
  ...
  if (!name) return textarea.focus();
  button.disabled = true;
  const original = button.innerHTML;
  button.innerHTML = "Caption yaratilmoqda…";
  try {
    const token = await user.getIdToken();
    const response = await fetch("/api/ai/caption", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ product: { name, description, category, price: sellPrice, features: [] }, language: "uz" }) });
