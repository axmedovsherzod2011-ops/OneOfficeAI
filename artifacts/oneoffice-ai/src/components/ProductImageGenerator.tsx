  ...
  return (
    <div className="mt-3 space-y-2.5">
      <button
        ...
        onClick={generate}
        disabled={busy}
        className="w-full flex items-center justify-center gap-2 rounded-xl border border-violet-400/30 bg-violet-500/10 hover:bg-violet-500/15 disabled:opacity-50 text-violet-200 py-2.5 text-sm font-medium transition"
      >
        {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
        {busy ? "Professional rasm tayyorlanmoqda…" : "+ Generate new"}
      </button>
      <p className="text-[11px] text-slate-500 text-center">
        Birinchi mahsulot rasmi AI uchun reference sifatida ishlatiladi. Oq fonli professional variant yaratiladi.
      </p>
      ...
      {error && (
        <div className="flex items-start gap-2 rounded-xl border border-rose-500/30 bg-rose-500/10 px-3 py-2.5">
          ...
          <button type="button" onClick={() => setError("")} className="text-slate-500 hover:text-white">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-2.5">
          ...
          <div className="flex gap-2 mt-2.5">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-violet-500 to-blue-500 text-white py-2.5 text-sm font-medium transition hover:opacity-90"
            >
            ...
            <button
              ...
              className="px-3 rounded-xl border border-white/10 bg-white/5 text-slate-300 hover:text-white transition"
