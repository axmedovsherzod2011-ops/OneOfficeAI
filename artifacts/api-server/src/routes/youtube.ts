...
async function buildFiveSecondVideo(imagePath: string, outputPath: string, isShort: boolean, product: any) {
...
router.get("/connectors/youtube/preview", handle(async (req, res) => {
  ...
  if (!cached?.videoData?.startsWith("v3:")) {
    ...
    try {
      ...
      const videoData = "v3:" + (await readFile(videoPath)).toString("base64");
      if (cached) {
  ...
  res.send(Buffer.from(cached!.videoData!.slice(3), "base64"));
}));
...
router.post("/connectors/youtube/publish", handle(async (req, res) => {
  ...
  try {
    ...
    if (cached?.videoData?.startsWith("v3:")) await writeFile(videoPath, Buffer.from(cached.videoData.slice(3), "base64"));
    else {
      const imagePath = join(tmpDir, "product.jpg");
      if (!(await downloadImage(images[0], imagePath))) { res.status(400).json({ error: "Mahsulot rasmini yuklab bo'lmadi." }); return; }
    ...
    const videoData = cached?.videoData?.startsWith("v3:") ? cached.videoData : "v3:" + (await readFile(videoPath)).toString("base64");
    ...
    res.json({ success: true, videoId, url: videoUrl, durationSeconds: VIDEO_DURATION_SECONDS, cachedVideo: Boolean(cached?.videoData) });
  } finally { await rm(tmpDir, { recursive: true, force: true }).catch(() => {}); }
}));
